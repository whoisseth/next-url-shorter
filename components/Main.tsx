    ...
    <>
      <form onSubmit={e => e.preventDefault()}>
        <div className=' absolute   top-[48rem]  bg-Violet z-40  rounded-lg flex flex-col gap-8 p-6 overflow-hidden inset-x-0 mx-7 md md:mx-20 md:top-[40rem] md:flex-row md:p-12 md:gap-4 '>
          <img
            className=' absolute -z-30 right-0 top-0 md:hidden'
          ...
          <div className='w-full'>
            <input
              type='text'
              className='relative w-full rounded h-12 px-4 placeholder-red-300  focus:ring-2 focus:ring-red-400 outline-none  md:rounded-lg'
              placeholder='Shorten a link here...'
            />
            <p className='absolute text-red-400 italic text-xs mt-2'>
              Please add a link{" "}
            </p>
          </div>
          <button className='text-white bg-Cyan font-semibold text-lg text-center py-2.5 rounded hover:shadow hover:bg-cyan-300 md:px-8 md:rounded-lg '>
            Shorten&nbsp;It!
          </button>
        </div>
      </form>
...
function Recognition({ image, title, desc, style }: propRecognition) {
  return (
    <>
      <div
        className={
          "bg-white p-4 rounded-md text-center pt-20 relative mb-20 md:w-[20rem] md:text-left md:p-8 md:pt-14  md:h-[14rem] " +
          style
        }
      >
